...
def test_limiter_passes_fuel_through_below_temperature_limit() -> None:
...
def test_limiter_reduces_fuel_above_temperature_limit() -> None:
    limiter = ExhaustTemperatureLimiter()
    ...
    assert protected_command.fuel_command == pytest.approx(0.6)
